...
class Vite
{
    ...
    public function __invoke($entrypoints, $buildDirectory = 'build')
    {
        ...
        $manifestPath = __DIR__ . $buildDirectory.'/manifest.json';
        ...
        foreach ($entrypoints as $entrypoint) {
            ...
            $tags->push($this->makeTag("{$buildDirectory}/{$manifest[$entrypoint]['file']}"));

            if (isset($manifest[$entrypoint]['css'])) {
                foreach ($manifest[$entrypoint]['css'] as $css) {
                    $tags->push($this->makeStylesheetTag("{$buildDirectory}/{$css}"));
                }
            }
